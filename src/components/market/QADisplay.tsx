...
export function QADisplay({ marketId, marketQuestion, marketDescription }: QADisplayProps) {
  ...
  const navigateToExtension = (extension: QANode) => {
    ...
    // When navigating to an extension, we need to include its full subtree
    const getCompleteExtensionTree = (node: QANode): QANode => {
      // Find any children that are also extensions
      const childExtensions = rootExtensions
        .filter(ext => ext.originalNodeId === node.id)
        .map(ext => getCompleteExtensionTree(ext));

      return {
        ...node,
        children: [...node.children.map(child => getCompleteExtensionTree(child)), ...childExtensions]
      };
    };

    const completeExtensionTree = getCompleteExtensionTree(extension);
    console.log('Navigating to extension with complete tree:', {
      extensionId: extension.id,
      originalNodeId: extension.originalNodeId,
      childrenCount: completeExtensionTree.children.length,
      fullTree: completeExtensionTree
    });
  ...
  const navigateBack = () => {
    ...
    if (previousTree) {
      console.log('Navigating back to previous tree:', {
        nodeCount: previousTree.length,
        rootIds: previousTree.map(n => n.id)
      });
