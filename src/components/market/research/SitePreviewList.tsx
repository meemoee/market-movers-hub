...
export function SitePreviewList({ results }: SitePreviewListProps) {
  ...
  return (
    <ScrollArea className="h-[200px] rounded-md border p-4 overflow-hidden">
      <div className="mb-2 text-sm text-muted-foreground">
        {results.length} {results.length === 1 ? 'source' : 'sources'} collected
      </div>
      {results.map((result, index) => (
        <div key={index} className="mb-4 last:mb-0 p-3 bg-accent/5 rounded-lg overflow-hidden">
          ...
          <a 
            ...
            className="text-xs text-blue-500 hover:underline block mt-1 break-all overflow-hidden text-ellipsis"
          >
            {result.url}
          </a>
        </div>
