...
export function SitePreviewList({ results }: SitePreviewListProps) {
  ...
  return (
    <div className="w-full max-w-full overflow-hidden">
      <ScrollArea className="h-[200px] rounded-md border p-4 w-full max-w-full overflow-x-hidden">
        <div className="mb-2 text-sm text-muted-foreground">
          {results.length} {results.length === 1 ? 'source' : 'sources'} collected
        </div>
        <div className="w-full flex flex-col space-y-4 max-w-full">
          {results.map((result, index) => (
            <div key={index} className="p-3 bg-accent/5 rounded-lg w-full overflow-hidden max-w-full">
