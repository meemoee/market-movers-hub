...
const HoverButton = React.forwardRef<HTMLButtonElement, HoverButtonProps>(
  ({ className, children, variant = 'buy', ...props }, ref) => {
    ...
    const createCircle = React.useCallback((x: number, y: number) => {
      ...
      const color = variant === 'buy' 
        ? `linear-gradient(to right, rgba(34, 197, 94, 0.5) ${xPos * 100}%, rgba(34, 197, 94, 0.8) ${xPos * 100}%)`
        : `linear-gradient(to right, rgba(239, 68, 68, 0.5) ${xPos * 100}%, rgba(239, 68, 68, 0.8) ${xPos * 100}%)`
    ...
    return (
      <button
        ref={buttonRef}
        className={cn(
          "relative isolate px-6 py-4", // Increased padding here
          ...
          "before:z-[1]",
          variant === 'buy' 
            ? "hover:bg-emerald-500/20 text-emerald-600"
