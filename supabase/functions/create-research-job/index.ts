...
async function performWebResearch(jobId: string, query: string, marketId: string, maxIterations: number, focusText?: string, notificationEmail?: string) {
  ...
  try {
    ...
    // Skip final analysis generation but log that we're skipping it
    await supabaseClient.rpc('append_research_progress', {
      job_id: jobId,
      progress_entry: JSON.stringify(`Skipping final analysis generation to speed up the process...`)
    });
    
    // Set an empty string for final analysis instead of generating it
    let finalAnalysis = "";
    
    // Create final results object with empty analysis
    ...
    try {
      ...
      const insightsPayload = {
        ...
        analysis: finalAnalysis, // Send empty final analysis
        marketId: marketId,
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert market research analyst who specializes in providing insightful
