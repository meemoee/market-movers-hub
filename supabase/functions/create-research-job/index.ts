...
async function performWebResearch(jobId: string, query: string, marketId: string, maxIterations: number, focusText?: string, notificationEmail?: string) {
  ...
  try {
    ...
    // --- Final Analysis Streaming Step ---
    await supabaseClient.rpc('append_research_progress', {
      job_id: jobId,
      progress_entry: JSON.stringify(`Generating final comprehensive analysis...`)
    });

    // Set status to 'finalizing' before starting the stream
    await supabaseClient.rpc('update_research_job_status', {
      job_id: jobId,
      new_status: 'finalizing'
    });
    console.log(`Job ${jobId} status set to finalizing`);
    ...
    try {
      ...
      if (allContent.length > 0) {
        // Generate final analysis, streaming updates to final_analysis_stream column
        finalAnalysisText = await generateFinalAnalysisWithStreaming(
          supabaseClient,
          jobId,
          allContent,
          query,
        ...
        finalAnalysisText = `No content was collected for analysis regarding "${query}".`;
        // Still update the stream column so frontend knows it's done
         await supabaseClient
            .from('research_jobs')
            .update({ final_analysis_stream: finalAnalysisText })
            .eq('id', jobId);
      }
    } catch (analysisError) {
      console.error(`Error generating final analysis stream for job ${jobId}:`, analysisError);
      finalAnalysisText = `Error generating final analysis stream: ${analysisError.message}`;
       // Update stream column with error
       await supabaseClient
          .from('research_jobs')
          .update({ final_analysis_stream: finalAnalysisText })
          .eq('id', jobId);
      ...
      await supabaseClient.rpc('append_research_progress', {
        job_id: jobId,
        progress_entry: JSON.stringify(`Error generating final analysis stream: ${analysisError.message}`)
      });
      // Note: We might want to fail the job here, but let's proceed to insights for now
    }

    // --- Structured Insights Step (after streaming final analysis) ---
    await supabaseClient.rpc('append_research_progress', {
      job_id: jobId,
    ...
    try {
      ...
      const insightsPayload = {
        webContent: webContentWithAnalyses, // Include previous analyses here
        analysis: finalAnalysisText, // Pass the completed final analysis text
        marketId: marketId,
        marketQuestion: marketQuestion, // Use the actual market question
    ...
    // Combine the final analysis text and structured insights
    const finalResultsPayload = {
      data: allResults, // Keep original search results if needed
      analysis: finalAnalysisText, // The complete final analysis text
      structuredInsights: structuredInsights // The insights object from extract-research-insights
    };

    // Update the job with the complete final results object in the 'results' column
    await supabaseClient.rpc('update_research_results', {
      job_id: jobId,
      result_data: JSON.stringify(finalResultsPayload) // Store the combined final payload
    });

    // Mark job as complete *after* final results are stored
    await supabaseClient.rpc('update_research_job_status', {
      job_id: jobId,
...
// Updates final_analysis_stream column incrementally
...
Present the analysis in a structured, comprehensive format with clear sections and bullet points where appropriate.`;
  ...
  try {
    ...
    // We will update the dedicated final_analysis_stream column
...
6. Draw comprehensive conclusions based on all available information`
        ...
        if (line.startsWith('data: ')) {
          ...
          try {
            ...
            if (jsonData.choices && jsonData.choices[0] && jsonData.choices[0].delta && jsonData.choices[0].delta.content) {
              ...
              // Update the final_analysis_stream column in the database every few chunks
              if (chunkSequence % 5 === 0 || chunkSequence === 1) { // Update on first chunk and every 5 after
                try {
                  const { error: updateError } = await supabaseClient
                    .from('research_jobs')
                  ...
                  if (updateError) {
                    console.error(`Error updating final_analysis_stream with streaming chunk:`, updateError);
                  } else {
                    // console.log(`Updated final_analysis_stream with streaming chunk ${chunkSequence}`);
                  }
                } catch (updateCatchError) {
                  console.error(`Caught error updating final_analysis_stream:`, updateCatchError);
                }
              }
            }
          } catch (parseError) {
    ...
    try {
      const { error: finalUpdateError } = await supabaseClient
        ...
        .eq('id', jobId);
      if (finalUpdateError) {
        console.error(`Error performing final update on final_analysis_stream:`, finalUpdateError);
      }
    } catch (finalUpdateCatchError) {
      console.error(`Caught error during final update of final_analysis_stream:`, finalUpdateCatchError);
    }
