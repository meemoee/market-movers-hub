...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, accept, x-stream',
};
...
// SSE Portfolio generation function
const generatePortfolioWithSSE = async (writer: WritableStreamDefaultWriter, content: string, supabaseAdmin: any) => {
...
serve(async (req) => {
  ...
  try {
    ...
    // For POST requests - SSE streaming or initial submission
    if (req.method === 'POST') {
      ...
      if (acceptHeader.includes('text/event-stream') || streamParam === 'true') {
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            try {
              ...
              // Continue with the portfolio generation process with SSE updates...
              // This will be the same logic as the GET request but with SSE streaming
              await generatePortfolioWithSSE(writer, content, supabaseAdmin);
        ...
        // Regular POST request - return success to indicate the job has started
